...
void Optimizer::visit(JoinPlan& join) {
  ...
  // Reorder children using SQLite-style beam search before building the
  // physical join tree. Children with lower estimated cardinality are placed
  // first so the left-deep join chain produces fewer intermediate rows.
  // See join_order.cc for the cost model and beam width formula.
  {
    auto order = beam_search_join_order(join.children, join.join_columns, tx_id);
    std::vector<std::unique_ptr<LogicalPlan>> reordered;
    reordered.reserve(join.children.size());
    for (size_t idx : order)
      reordered.push_back(std::move(join.children[idx]));
    join.children = std::move(reordered);
  }
