...
// Unescape a raw STRING token body (quotes already stripped).
...
Constant get_constant(IIC3413DBParser::ConstantContext* ctx) {
  if (ctx->STRING() != nullptr) {
    auto raw = ctx->STRING()->getText();
    raw = raw.substr(1, raw.size() - 2); // strip surrounding double quotes
    auto string = unescape_string(raw);
...
std::any QueryPreprocessor::visitLikeExpr(IIC3413DBParser::LikeExprContext* ctx) {
  ...
  // Unescape: standard escapes become their byte values; \% and \_ are kept
  // as two-char sequences so the matcher can treat them as literal % / _.
  auto pattern = unescape_string(raw);
