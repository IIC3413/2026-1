...
int main() {
  std::string table_name = datasets::DELETE_2;
  {
    ...
    hash_index.delete_record(RID(0, 4));
    hash_index.delete_record(RID(0, 22));
    hash_index.delete_record(RID(0, 27)); // last readable record
  }
  // Compare index with reference using compare_index
  std::string cmd =
      std::string("build/Debug/bin/compare_index ") + table_name + " --mode page --keys 5 --score 0.5";
