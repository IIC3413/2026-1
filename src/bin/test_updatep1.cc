...
2) It verifies that the old versions of the records still have correct next_rid links to the next version in the chain.
3) It verifies that all versions in the chain (including the last updated version) have the correct first_rid link pointing to the original root record.
//TODO: Test
*/
...
int main() {
	...
	if (!fs::exists(source_db) || !fs::is_directory(source_db)) {
		...
		std::cout << "Score: 0/2" << std::endl;
		return EXIT_FAILURE;
	}

	int score = 0;
	bool old_ref_ok = true;

	{
		...
		std::cout << "Verifying next_rid references..." << std::endl;

		for (const RID& root_rid : roots) {
			const auto& chain = chains[root_rid];
			bool this_old_ref_ok = true;
			for (size_t i = 0; i + 1 < chain.size(); ++i) {
				const auto header = heap_file->get_record_header(chain[i]);
				if (header.is_invalid() || !(header.next == chain[i + 1])) {
					this_old_ref_ok = false;
				}
			}
			...
			std::cout << "Chain from RID(" << root_rid.page_num << "," << root_rid.dir_slot << ") length "
								<< chain.size() << std::endl;
			std::cout << "  old versions next_rid links: " << (this_old_ref_ok ? "OK" : "FAIL") << std::endl;
		}
	}
	...
	std::cout << "Old version next_rid check: " << (old_ref_ok ? "PASS (+1)" : "FAIL (+0)") << std::endl;
