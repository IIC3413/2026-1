...
void TransactionManager::commit_transaction(TxID tx_id) {
  ...
  try {
    ...
    for (auto [table_id, delta] : transaction->cardinality_deltas) {
      if (delta != 0) {
        catalog.update_cardinality(table_id, delta);
      }
    }
    ...
    // commit_transaction() already has the transaction
...
void TransactionManager::update_cardinality_delta(TxID tx_id, TableId table_id, CardinalityAction action) {
  std::unique_lock lock(active_transaction_mutex);
  auto it = active_transactions.find(tx_id);
  if (it != active_transactions.end()) {
    Transaction& transaction = *it->second;
    transaction.update_cardinality_delta(table_id, action);
  }
}
